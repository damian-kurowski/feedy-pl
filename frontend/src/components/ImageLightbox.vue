    ...
    >
      <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 p-4 max-h-[80vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-3">
          <h3 class="text-sm font-medium text-gray-700">Zdjecia produktu ({{ images.length }})</h3>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div v-for="(img, idx) in images" :key="idx">
            <img :src="img" :alt="`Zdjecie ${idx + 1}`" class="w-full h-auto rounded border border-gray-200" loading="lazy" />
          </div>
        </div>
      </div>
    </div>

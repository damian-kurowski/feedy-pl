...
async function deletePost(id: number) {
  ...
  undoableDelete({
    message: `Usunięto wpis „${snapshot.title}"`,
    localRemove: () => { posts.value.splice(idx, 1) },
    localRestore: () => { posts.value.splice(idx, 0, snapshot) },
    commit: () => api.delete(`/admin/blog/${id}`).then(() => undefined),
      ...
      <section v-if="editing" class="bg-white border border-gray-200 rounded-2xl p-5 sm:p-6 mb-8">
        ...
        <div v-if="showPreview" class="mb-6 border border-gray-200 rounded-xl bg-gray-50 p-5 sm:p-8">
          ...
          <article>
            ...
            <div class="blog-prose" v-html="editing.html || '<p class=\'text-gray-400\'>(brak treści)</p>'"></div>
          </article>

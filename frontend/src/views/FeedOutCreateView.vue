...
async function handleCreate() {
  ...
  try {
    ...
    error.value = e.response?.data?.detail || 'Błąd podczas tworzenia feedu'
...
<template>
  <div class="max-w-3xl mx-auto py-10 px-4">
    <div class="flex items-center gap-2 mb-8">
      <div v-for="s in 3" :key="s" class="flex items-center gap-2">
        ...
        <span class="text-sm" :class="step >= s ? 'text-gray-900' : 'text-gray-400'">
          {{ s === 1 ? 'Platforma' : s === 2 ? 'Informacje' : 'Utwórz' }}
        </span>
    ...
    <div v-if="step === 1">
      <h1 class="font-heading text-2xl font-bold text-gray-900 mb-2">Wybierz platformę</h1>
      <p class="text-sm text-gray-500 mb-6">Na jaką platformę chcesz wygenerować feed produktowy?</p>
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <button v-for="p in platforms" :key="p.platform"
          class="border-2 rounded-2xl p-6 text-left cursor-pointer transition-colors hover:border-indigo-300 hover:bg-indigo-50"
    ...
    <div v-if="step === 2 && platformInfo">
      <h1 class="font-heading text-2xl font-bold text-gray-900 mb-2">{{ platformInfo.name }}</h1>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-indigo-50 border border-indigo-200 rounded-2xl p-6">
          <h2 class="text-sm font-semibold text-indigo-800 mb-2">Wskazówki</h2>
          <ul class="space-y-1">
            <li v-for="(tip, idx) in platformInfo.tips" :key="idx" class="text-sm text-indigo-700 flex gap-2">
      ...
      <div class="flex items-center gap-4 mt-8">
        <button class="bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-xl px-4 py-2 transition cursor-pointer" @click="goBack">Wstecz</button>
        <button class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl px-5 py-2.5 transition-all hover:shadow-lg hover:shadow-indigo-500/20 cursor-pointer" @click="step = 3">Dalej</button>
      </div>
    </div>

    <div v-if="step === 3">
      <h1 class="font-heading text-2xl font-bold text-gray-900 mb-6">Utwórz feed wyjściowy</h1>
      <div v-if="error" class="mb-4 p-3 bg-red-50 border border-red-200 text-red-700 rounded-xl text-sm">{{ error }}</div>
      ...
      <div class="flex items-center gap-4">
        <button class="bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-xl px-4 py-2 transition cursor-pointer" @click="goBack">Wstecz</button>
        <button :disabled="loading" class="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-sm font-semibold rounded-xl px-5 py-2.5 transition-all hover:shadow-lg hover:shadow-indigo-500/20 cursor-pointer" @click="handleCreate">
